...
model_name = 'wesee7_3.pt'

opt = easydict.EasyDict({'agnostic_nms':False, 'augment':True, 'classes':None, 'conf_thres':0.25, 'device':'', 'exist_ok':False, 'img_size':640, 'iou_thres':0.45, 'name':'exp',
                            'no_trace':False, 'nosave':False, 'project':'runs/detect', 'save_conf':True, 'save_txt':True, 'source':'',
                            'update':False, 'view_img':False, 'weights':[model_name]})
...
async def file_upload(img: bytes = File(...), sequenceNo: int = 1):
    ...
    # logger.info(f"image recieved! size: {im.size}, image conversion time: {time.time() - tick}")
    ...
    dataset = LoadImages('inference/images/zidane.jpg', img_size=imgsz, stride=stride)  # 이부분 좀더 만져야됨
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            results["yolo"]=boxes
            print(results)
            result_dict[sequenceNo] = results
    ...
    logger.info("발견된 물체: {}", [ result['name'] for result in result_dict[sequenceNo]['yolo'] ])

    stateMachine.newFrame(result_dict[sequenceNo]['yolo'])
